...
with open("./auth/trailCreds.yaml") as file:
...
def tabForm(tab, tabRef):
    ...
    with tab.expander("Subcontractor"):
        ...
        if st.toggle("Electrical Contractor", key=f"elecCon@{tabRef}"): #and deployment strategy
            tabSubCon['elecContractor']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"elecCon#amount@{tabRef}")
        ...
        if st.toggle("Dry Wall", key=f"drywall@{tabRef}"): #and deployment strategy
            tabSubCon['dryWall']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"drywall#amount@{tabRef}")
        ...
        if st.toggle("Trenching & Excavation", key=f"trench@{tabRef}"): #and deployment strategy
            tabSubCon['trenching']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"trench#amount@{tabRef}")
    ...
    with tab.expander("Licensing"):
        ...
        tabLicense['elecPermit']['amount'] = st.number_input(label = 'Amount', min_value=0, value=125, key=f"elecPermit#amount@{tabRef}")
        ...
        tabLicense['buildingPermit']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"buildingPermit#amount@{tabRef}")
        ...
        tabLicense['surveyorFee']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"surveyorFee#amount@{tabRef}")
        ...
        tabLicense['adminFee']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"adminFee#amount@{tabRef}")
        ...
        tabLicense['masterElecFee']['amount'] = st.number_input(label = 'Amount', min_value=0,value=300, key=f"masterElecFee#amount@{tabRef}")
    ...
    with tab.expander("Wall Penetrations & Concrete Work"):
        ...
        if st.toggle("Wall Penetrations", key=f"wallPene@{tabRef}"):
            tabWallPenetration['wallPenetration']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"wallPene#amount@{tabRef}")
        ...
        if st.toggle("Concrete Work", key=f"concWork@{tabRef}"):
            ...
            tabWallPenetration['ConcreteFastening']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"concFast#amount@{tabRef}")
            ...
            tabWallPenetration['ConcreteFooter']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"concFoot#amount@{tabRef}")
            ...
            tabWallPenetration['ConcreteLabor']['amount'] = st.number_input(label = 'Amount', min_value=0, key=f"concLabor#amount@{tabRef}")
    ...
    with tab.expander("Trenching and Excavation"):
        ...
        for addon in tabTrenching:
            ...
            tabTrenching[addon]['amount'] = st.number_input(label=f"{addon} Amount", min_value=0, key=f"{addon}#amount@{tabRef}")
    ...
    with tab.expander("EVCEs + Add Ons"):
        ...
        for item in tabEVCE:
            st.write(f"{item}")
            tabEVCE[item]['amount'] = st.number_input(label=f"{item} Amount", min_value=0, key=f"{item.replace(' ', '')}#amount@{tabRef}")
